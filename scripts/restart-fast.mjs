// 触发一次 fast 换代（?cmd=restart）并轮询到落定。
// 用途：验证 (a) 运行中的 switchboard 是否已是最新 build（响应里应有 fast:true）
//        (b) fast 路径是否真的跳过了 defer（对比 state.jsonl 的耗时）
//        (c) 新 gen 是否健康、旧 gen 是否被 retire
import fs from 'node:fs'

const ADMIN = 'http://127.0.0.1:31800/'
const OUT = 'D:/project_develop/dsh-brain/out/restart-fast.txt'
...
const stateLineCount = () => {
  try { return fs.readFileSync(STATE, 'utf8').split('\n').filter((l) => l.trim()).length } catch { return -1 }
}
...
const t0 = Date.now()
const kick = await get('?cmd=restart')
...
try {
  ...
  for (const l of lines.slice(before)) {
    try {
      ...
      say(`  t=${r.t} stage=${r.stage} gen=${r.gen} note=${r.note}`)
    } catch { say('  ' + l) }
  }
