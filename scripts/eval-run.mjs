...
function taskPrompt(t) {
  return [
    ...
    // ★★ w57（选项 B）：题面**不再给验证命令**。
    //   旧句 `验证命令（你可以自己先跑）：<oracle cmd>` 有两个毛病：
    //     ① 它点名了判据脚本，而该脚本按 R1 **不在被测树里**（题面让 agent 去跑一个不存在的文件）；
    //     ② 换个臂、换套题，"哪条命令算判据"就变了 ⇒ 题面把一次实验的**装置细节**写死了。
    //   现在只留一句不含判据根路径、不点名任何 oracle 的要求：验证方式由 agent 自己决定。
    '请自行确认你的改动确实修好了问题。',
  ].join('\n')
}
