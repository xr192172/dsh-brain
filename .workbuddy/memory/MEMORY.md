...
- **★ 启动器（launcher）**：用户 2026-09-26 提的**新主线** —— 造一个"像游戏启动器那样、本身能管代数"的入口。
  ...
  ★ 三份（按读序）：
  ① `docs/launcher-function-list-2026-09-26.md` —— **F0–F5 功能清单**（只列功能，铁律 37 首个执行样本）；
  ② `docs/launcher-sentinel-model-2026-09-26.md` —— **用户三条裁决逐字存档** + 哨兵模型形式化 + 施工图纸确定性实测；
  ③ 仪器 `scripts/canvas-determinism-probe.mjs` —— 照抄源码 `featureIdOf`，同目录跑两遍逐字节比对
     （实测：`design-canvas/src` **308 文件 → 17 功能，两遍逐字节相同**）。
  ★★ **哨兵模型（用户裁决）**：`POOL = {primary, sentinel, others}`；**立哨 / 选定 / 提拔** 三动作；
     **提拔仅由"主代退役"触发**（模型里**没有**"算哪个最好"这一步）。
  ★★ **唯一真缺口**：`?cmd=handover` 把 **"立哨"与"提拔"合成一步** ⇒ 需拆成两个动作
     （**与 R2 拆"准备 vs 换代"同族，有先例可抄**）。
  ★★ **纪律**：判据只许落在**结构层**（确定性）；`project_docs`/`llm_decider`（**193 个工具文件里仅这 2 个调 LLM**）
     出来的东西**一律标非确定性、不得当判据**。
  ★ 待用户裁决**只剩两条**：Q-甲/乙（页面随点随起 vs 常驻；我倾向**随点随起**）
     / Q-新（立哨与提拔要不要拆；我倾向**拆**）。
