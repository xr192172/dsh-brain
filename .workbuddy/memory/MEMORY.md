...
38. **★★★ 模块级"空模板"常量【不许浅拷贝返回】** —— `return { ...EMPTY }` 里若含数组，
    那个数组会在**同进程所有实例间共享** ⇒ 每个实例的写入都追加到同一份 ⇒
    **重复条数 == 进程内已建实例数**（实测坐实）。
...
39. **★★★ 不许在【用户桌面】上起任何可见窗口做实验**（2026-09-26 真事故，用户投诉）。
    · 我为了测"杂讯是否只在有 console 时发作"，跑了 `start "" /wait <file>` **80 次**
      ⇒ 给用户弹了**满屏关不掉的模态对话框**（conhost 的系统框，点确定/取消/叉**都会立刻再弹**）。
    · ⇒ **纪律**：`start` / GUI / 任何会弹窗的东西，**一律不许**在用户桌面上做实验。
      需要"有控制台的环境"就**重定向到 `out/` 日志**；真要弹窗**必须先说、先征得同意**。
    · ★ 顺带：**换通道前先证明新通道能看见阳性对照**。我拿
      `spawnSync(cmd,[...])`（**无 console**，`/c chcp` 回空）测 4 臂 × 150 次全 0，
      就写"症状复现不出" ⇒ **那是假绿**（撞铁律 15）。
    · ★ 再顺带：**一次改多个变量后不许归因**。我同时改了"非 ASCII/注释/行数"三样，
      就拿"改完不响了"当"病因=非 ASCII"的证据 ⇒ 单变量隔离 4 臂全 0 ⇒ **归因无效**。
    · ★ 收尾三条可复用工具：`out/_find-dialog.mjs`（枚举可见顶层窗口找弹窗主人；
      **注意 Win32 API 必须显式 CharSet.Unicode，否则标题全变成单字符**）、
      `out/_check-probes.mjs`（查有无残留探针进程）、`out/_kill-orphan-conhost.mjs`
      （**只杀父进程已消失的** conhost，活父的**一律不动**）。
