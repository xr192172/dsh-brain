...
1. **`pass^k`**：同题重复 3 次（同臂），看方差 —— 现在 k=1，单次差异可能吞掉真实差别。
2. **难度 / 预算**：三题已饱和（成功率两臂都 100%）⇒ 要么写更难的题（多文件、无现成门当 oracle），
   要么把 `budget.maxToolCalls` 从 60 收到 **8–12**（实测单臂用 7–20 次 ⇒ 立刻有区分度）。
3. **更极端的臂**：`minimal`（只有 bash + `str_replace_editor` 两工具）当挑战者，照出"工具面宽窄"的代价。
