...
>
> ★ **2026-09-20 17:55 追加：架构层已给出一条与之直接相关的定案** ——
> 见 `docs/revised-architecture-2026-09-20.md` §5.1/§5.2：**下层三格形状已定**
> （隔离被测 = 工具；**生产、进化 = 独立的子 Agent**，理由 = **防串供**）。
> ⇒ 对本文的直接影响：**L2~L4 的"判据持有者"必须是「进化子 Agent」这个独立主体 +
> 独立于被改者的 holdout**；**不能**把 L2~L4 做成"被同一个 agent 调用的普通工具"，
> 否则就是"被测者给自己判分"（假绿工厂）。
> ⇒ 另外 `hard-switch` 那个未拍板项（`master-plan §11.3`：holdout 存哪、谁维护）现在**有了确定它的前提**
> —— 先定"进化子 Agent 用什么上下文/holdout 谁持有"，再谈 L3 怎么做。
