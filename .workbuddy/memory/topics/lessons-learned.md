...
## 附录 A — MEMORY.md 2026-09-26 瘦身前正文存档（未删一字）

> 2026-09-26：MEMORY.md 再次超过注入上限（58KB）⇒ 做第二次瘦身。
> 为遵守"**索引里没有 = 不存在**"，把瘦身前正文**全文**追加在这里（**只读**，不作为日常入口）。
> 日常入口 = 新版 MEMORY.md（一行式铁律 + 主题索引）。
...
- ★★ **入口脚本不许"位置相关"**：`dsh-up.cmd` 拷到桌面后 `%~dp0..` = `C:\Users\Admin\` ⇒ 崩。
  ⇒ 桌面那份必须**打上绝对仓库路径**（`install-desktop-icon.mjs` 负责写 `set "DSH_REPO_OVERRIDE=…"`）。
  ★ 装机器里的坑：替换正则**跨换行**会吞掉下一行；"坏行检测"**判据太宽**会把正常行判坏（假红）。
- ★★ **判据要带"正例"**：我管理面把臂路径写错（`wt/_arms` vs `wt/../_arms`）⇒ **误拒真臂**，
  而当时只有"不存在的臂被拒"那条 ⇒ **抓不到**。⇒ 补 **④b「真存在的臂必须被接受」**。
  ⇒ 通用：**每条"拒绝类"判据都配一条正例对照**，否则"全拒"也能过。

---

## 铁律 27–32 全文与证据（2026-09-26 新增；MEMORY.md 里只留一行式）

> ★ 编号**不复用**；MEMORY.md 的"铁律"段是**一行式索引**，**全文在此**。

### 27. ★★★ 派活给"会写同一仓库"的 agent 之后，提交前必须 `git diff <该文件>` 看内容

**`git status` 只告诉你"文件名是不是你的"，不告诉你"内容是谁写的"。**

**实证（2026-09-26）**：DSH 在 **14:25:05** 改完了我新建的文件；我在 **14:26:35** `git add` + 提交
⇒ **它的改动被装进我的提交、且提交信息里没署名**。`git status` 当时显示的是"M 我的新文件"，
看着完全像"我的改动"。

**纪律**：
- 提交前对该文件跑 `git diff <file>`，**读一遍内容**；
- 若里面有别人的改动 ⇒ 要么**单独提交并署名**，要么在提交信息里写明"含 X 在 HH:MM 的改动"。

### 28. ★★ 判据不许是【同义反复】

**"输入被清空时输出当然为空"** 的那种判据（"清空 map ⇒ role 为空"、"扫空目录 ⇒ 空数组"）
**不含任何信息量**，却会让"12/12 全过"看起来更可信 = **假绿**（与铁律 21 同族）。

**判别法（一句话）**：**问"这条判据在什么输入下会红？"** —— **答不出来就是同义反复。**

**实证**：
- 2026-09-26 第一棒：DSH 给 `self-dev-brief.mjs` 新增了 2 条**同义反复判据**，
  我读 diff 时发现并定性（`docs/verify-self-dev-first-leg-2026-09-26.md`）。
- 2026-09-26 第三棒：DSH 在 `arm-up.mjs` 的消融里写了
  `const ctrlFaceSkip = true && true === true` ⇒ **恒 true、永不红** ⇒ 那条"消融"证明不了任何事。
  ⇒ **我在它交卷前就指出**（见下"发起方自用的教训"）。

**★ 配套（本次同时抓到的那条真缺陷）**：**"扫全目录" ≠ "精选入口"**。
把一份**精选清单**换成**目录倾倒**，是把"说明书"降级成"噪音"
（实测：165 条里 159 条 `role` 为空，agent 得先自己过滤）。
⇒ **交付物要保持用户原话里的粒度**（"我们**写**的那些" ≠ "目录里**有**的那些"）。

### 29. ★★★ 子 agent 的"读数"与它的"归因"必须【分开验】

**读数**（它贴的 stderr 原文）它抄对了；**归因**（"因为 X 所以 Y"）它编错了。

**实证（2026-09-26 第二棒）**：同一个失败，DSH 给了**两条错归因**：
1. 把"⑦ 探的是 `:3080`"说成"**`:31800` 不稳**"（**指错了端口** —— ⑦ 只探 `:3080`，见 `arm-up.mjs:376`）；
2. 自检**编号整体错位一位**（它写 `✓④两席 / ✓⑤池 / ✗⑥现役`，真值是 `✅⑤两席 / ✅⑥池 / ❌⑦现役`），
   且**④ 掉了**。

**纪律**：**归因也是结论，也要证据**。验子 agent 的交付时——
**只信"可复算的读数"**（它贴的原文你能拿去对），**不许信它串好的因果**。
⇒ 核验动作 = **拿它贴的原文，自己重新推一遍因果**。

**★★ 同一条对【发起方自己】同样成立（2026-09-26 我连续改口）**：
僵尸 lease 事故里，三路读数（`lease.json` / 控制面 `?cmd=status` / `netstat`+`tasklist`）
**两版都一样、都是对的**；变的**只是我对它的解释**：
- 第一次："旧代从不停"（**方向对**，但处方"叫用户手停 pid 16520"**有害** —— 16520 才是唯一在正常服务的）；
- 中途改口："只是文件陈旧、实例其实健康"（**错** —— `gen EXIT pid=9224 code=1` 证明旧代确实没退、新代确实死了）。
⇒ **读数与归因必须分开陈述**；**归因错一次就会给出有害的处方**。

### 30. ★★★ "健康检查判红" 与 "被检对象坏了" 是两件事

见红**先问三句**：
1. **它探的是谁**？（实测：都写着"现役"，其实**只探 `:3080`**）
2. **探针本身够不够硬**？（实测：**无超时、无重试**的探针，在**同机负载**下 **5/6 次假红**）
3. **被探对象此刻在干什么**？（实测：现役正在跑"**发起这次检查的那个会话**" ⇒ **自干扰**）

★ **越"来自我实验"越假红** ⇒ `不敢发题` ⇒ **整条链自锁**。

**修法不是"重试到绿"**（那是把假红转成假绿，违反铁律 7/15），而是：
**探针加超时 + 有界重试，并把结果【分类】** `ok` / `slow` / `unreachable`，
**判据分辨**"探不通（通道不可用，看不到）"与"答不对（看到了坏结果）"
—— 这两件事的处置完全不同（铁律 12/14）。

**实证**：30 并发压 `:3080` + 同款探针 ⇒ **5/6 次 `http:0 (TimeoutError)`**；
加超时(20s)+重试(5 次/间隔 2s) 后 ⇒ `ok / kind=ok / tries=1`。

### 31. ★★★ 单一写者的文件，外部进程【只读、绝不写】

**实证（2026-09-26，我在【任务书】里自己犯的）**：我一度要求 DSH
「② **修复 `lease.json`**（把它清成无活跃代）」。**这是错的，已撤回。**

**为什么错** —— `packages/switchboard/src/lease.ts:6` 的 docstring 逐字：

> 仅 Switchboard（协调器）持有并写本文件——这就是"单一写者"的物理锚点。

外部脚本（`arm-up.mjs`）去 rewrite 它会：
1. **破坏单一写者不变量**；
2. 与活着的 switchboard 形成 **read-modify-write 竞态**
   （跨进程 `commit()` 用 `renameSync` 是**原子替换**，但跨进程 `read→write` **不是原子的**）；
3. **重置 `writerToken` / `lastFencingSeq`** —— 那正是设计里用来**防 split-brain 的 fencing token**。

**正解**：**只读**。陈旧状态让**该组件的下一个实例自己在启动路径上恢复**
（`main.ts:178-189` 的 `coord.getLease().clear()` —— 那是**唯一合法的清 lease 通道**）。

★ **推论（这条比上面更通用）**：**"在报错里叫人去手改/手杀"之前，先确认那东西是不是别人的。**
我那次若真照自己写的做，就会**打死那个唯一健康的实例**（pid 16520 一直在正常服务）。

### 32. ★★★ "诊断对不对" 要用【自洽性】验，不只验读数

**两路读数互相矛盾时**（`lease.json` 说 `pid=9224` 死／活着的控制面说 `pid=16520` 活），
**先问"谁在写它、谁在读它"，而不是先选一边信。**
**真相由【写入时序】决定，不在任一快照里。**

**决定性证据来自日志的行序**（`_arms/a/dshhome/logs/switchboard-run.log`）：
```
3: [switchboard] lease recovery OK: gen=gen-33082 port=33082 pid=16520 alive   ← 健康
8: [switchboard] lease recovery OK: gen=gen-33082 port=33082 pid=9224 alive    ← 又起一代
9: [switchboard:spawn] gen EXIT gen=gen-33082 pid=9224 code=1 …                ← 新代当场死
```
⇒ 死因在 `boot.log`：`33101 被占` + `EADDRINUSE 127.0.0.1:33191` ⇒ `crash(exit 1)`。

★ **"日志的行序"本身是一等证据** —— 比任何单点快照都强。
★ 推论：**排查"文件说 A、服务说 B"这类矛盾时，第一步是找"谁在什么时候写了它"**，
  也就是**带时间戳的追加日志**，而不是再多读几遍那两个快照。

---

## 铁律 33–34 全文与证据（2026-09-26 晚间新增；MEMORY.md 里只留一行式）

### 33. ★★★ 【文案诚实地写了"不确定"】≠【判据诚实地处理了"不确定"】

**事故**：`scripts/arm-up.mjs:205-210` 的 ⑦（"现役仍健康"）写了一段非常清醒的文案：
...
**但 `ok` 仍然取 `liveOk === true`** ⇒ `unreachable` 判**红** ⇒ 下游 `run-experiment.mjs` 见
"自检全过"为假 ⇒ **`[失败] 起一代没过自检 ⇒ 不敢发题`** ⇒ 整条自实验链被一个**通道抖动**掐死。

**实测（两次完全复现）**：
```
evals/runs/_mgmt/t1-guard-1790411194926.json   ✅⑤ ✅⑥ ❌⑦  code=1
evals/runs/_mgmt/t1-guard-1790411476806.json   ✅⑤ ✅⑥ ❌⑦  code=1
```

**★ 教训**：`kind` 已经被正确算出来了（`ok`/`slow`/`unreachable` 三分），
**但没有任何消费者读它** —— `detail` 里那句提醒是**给人看的**，`ok` 才是**给机器看的**。
⇒ **判据的"不确定"必须能被下游【读到并分流】；只写进文案 = 等于没分类。**

**修法方向（未实施）**：`ok` 三分 ——
- `true`：探到且答对；
- **`unknown`**（`unreachable`）：**不判红**、显式记 `unknown`、**带重试复探**；最终仍 unknown 则标"未能判定"；
- `false`：**探得通但答错**（这才是"被检对象坏了"）。

★ 两端都错得一样重：**`unknown` 当"通过" = 假绿（铁律 14）；`unknown` 当"失败" = 假红**（把通道问题算成被检对象的错）。

---

### 34. ★★★ 判断"现役被影响了"之前，先排除【控制面自己把通道占了】

**机制**：`packages/switchboard/src/mgmt.ts:159`
```js
const r = spawnSync(argv[0], argv.slice(1), { encoding:'utf8', timeout: 900_000, cwd: wt, env: childEnv(env) })
```
`spawnSync` **同步阻塞事件循环** ⇒ 派活（跑 `arm-up`，几十秒）期间，**控制面自己的代理转发被拖住**。

**并发实测**（一边 0.2s 间隔打 `:3080 session.list`，一边经 `:31800` 发**非 dry** `experiment`）：
```
n = 90    ok = 83    err = 7（全部 "timed out"）
p50 = 0.52s    p95 = 0.66s    ★ max = 8.23s
```
**旁证**：我自己的轮询器同期连续 3 次 `timed out`（≈90s）。

**★ 单因素消融（决定性）** —— 同机器、同探针、同代码，**唯一自变量 = "arm-up 被谁拉起"**：
...
**★ 教训**：**"探不通"的最常见原因是【探查者与被查者共用了那个正在忙的进程】**，不是被查者坏了。
⇒ 见红先问一句：**"这个探针和那个被探的东西，是不是同一条事件循环？"**

---

### 附：同棒发现的"修了但没部署"缺陷（不单独占用铁律编号）

`packages/switchboard/src/mgmt.ts`（mtime **16:04**）已含 `childEnv()` + `ARM_IDENTITY_ENV_KEYS`（修 `DSH_HOME` 泄漏），
...
⇒ ★ **"源码里有修复" ≠ "在跑的进程里有修复"** —— 判"某缺陷修没修"，判据必须落在
**在跑的那份 bundle**（`grep` 它 + 看它的启动时间），而不是源码文件的 mtime。
（这与铁律 18"回读成功 ≠ 生效的是你以为的那份"同族；此处是**构建产物**那一层。）

---

## 铁律 35–40 全文与证据（2026-09-26 深夜新增；MEMORY.md 里只留一行式）

### 35. ★★★ 问设计岔路之前，必须先把【当前命令面/现状】摆出来

**用户逐字**：*"我不能理解，我不知道现在有哪些命令。"*

⇒ 用户**没法裁决**，不是因为问题难，是因为**我没先给现状**。

**纪律**：凡"该选甲还是乙"，先摆 ① 现在有哪些动词 ② 各起什么效果 ③ 改了会动到什么，**再**问选哪个。
★ 与铁律 37（先落编号清单）同族：**都是"别让用户在雾里做决定"。**

---

### 36. ★★ 逃生阀必须有台账

`isolated-instance --force` 从"应急阀"**长成了常规路径**（一条旁路、六个症状）。

⇒ 任何"平时不该用"的开关：① 默认关闭且**要显式动词**触发 ② **每次使用记账留痕** ③ 上线时就要能被审计。

★ 与铁律 9（否定+禁止 ＞ 说明+判断）、21（自批要留痕）同族：**无声的例外 = 未来的旁路。**

---

### 37. ★★★ 填细节前，功能必须先落成【带编号的清单】

**用户逐字点破（2026-09-26）**：
> *"我往里面填细节老是会填出和原……和其他细节偏离的东西，连功能都有点对不上。"*

**四条机制**（为什么我会偏）：
1. 功能只活在对话里 ⇒ 我参考的是**我上一条消息**，偏差**累乘**；
2. 我天然往"最小可行修法"滑 ⇒ 细节被**现有结构**锁死；
3. 自检按**我实现的样子**写 ⇒ **A′ 从没跟功能清单对过 = 假绿**；
4. 我**从不主动报偏** ⇒ 偏差全靠用户发现。
...
### 38. ★★★ 模块级"空模板"常量【不许浅拷贝返回】

`return { ...EMPTY }` 里若含数组，那个数组会在**同进程所有实例间共享** ⇒
每个实例的写入都追加到同一份 ⇒ **重复条数 == 进程内已建实例数**（实测坐实）。

**症状极难认**：**单实例单测永远全绿**。本仓库 `test-sentinel-pool.mjs` 逐用例 `new` 才逼出来
（⑤/⑥ 恒红，而"同样场景单独跑"全绿 —— **判据只报红、不报"为什么红"，就是这条的成因**）。

- **识别口径**：`a.others === b.others`（两个**不同目录**的实例）为 `true` ⇒ 确诊。
- **修法**：改成 `freshEmpty()` **工厂**（每次新建字面量）+ 读回时 `.slice()` 复制；
  `current`/`commit` 一律返回**快照**而非活的内部对象；**删掉常量**，从形态上消灭可能。
- **扫同族**：`scripts/scan-shallow-share.mjs`（找 `return { ...CONST }` × 模板定义 的交叉）。
  当前仅剩 `lease.ts`（全原始值 + `activeGen` 每次整体替换 ⇒ 安全但脆，已注释钉死）。
- ★ **"安全"要说明它为什么安全、以及加什么字段就不再安全** —— 否则下一个人加一个数组字段就复现。

**同族通用律**：**判据红了先问"它是不是只在一个实例/一种调用顺序下才红"**；
**"换个跑法就绿"是共享状态的指纹，不是巧合。**

**消融自证**：改回 `{ ...EMPTY }` ⇒ 12 ok / 4 FAIL；恢复 ⇒ 16 ok / 0 FAIL。
★ 顺带修掉一个**哑门**：⑯ 原先只认常量名 `EMPTY`，消融改名 `__T2` 后仍假绿 ⇒ 改为认**字面量形态**。

---

### 39. ★★★ 不许在【用户桌面】上起任何可见窗口做实验

**2026-09-26 真事故**。用户逐字：
> *"你在干嘛，我这边一直弹弹窗说找不到两道斜杠的一个路径，而且我点确定或者点取消或者点叉掉它都一直在弹。"*

我为了测"杂讯是否只在有 console 时发作"，跑了 `cmd /c start "" /wait <file> --stop` **80 次**。
`start` 每次都**新开一个控制台窗口**，其中失败的会弹 **conhost 的系统级模态框** ——
**点确定 / 取消 / 叉都会立刻再弹**，普通办法关不掉。

⇒ **纪律**：`start` / GUI / 任何会弹窗的东西，**一律不许**在用户桌面上做实验。
需要"有控制台的环境"就**重定向到 `out/` 日志**；真要弹窗**必须先说、先征得同意**。

**同棒另外两个错（都是假绿）**：
- ★ **换通道前先证明新通道能看见阳性对照**。我拿 `spawnSync(cmd,[...])`（**无 console**，
  `/c chcp` 回**空**，正常应回 `活动代码页: 936`）测 4 臂 × 150 次全 0，就写"症状复现不出"
  ⇒ 拿一个**根本不具备触发条件**的通道去测，得到的 0 毫无意义（撞铁律 15）。
- ★ **一次改多个变量后不许归因**。我同时改了"非 ASCII / 注释 / 行数"三样，
  拿"改完不响了"当"病因 = 非 ASCII"的证据 ⇒ 单变量隔离 4 臂全 0 ⇒ **归因无效**。

**收尾三条可复用工具**：
- `out/_find-dialog.mjs` —— 枚举可见顶层窗口找弹窗主人。
  ★ **Win32 API 必须显式 `CharSet=CharSet.Unicode`**，否则标题全变成单字符（我第一版就踩了）。
- `out/_check-probes.mjs` —— 查有无残留探针进程。
- `out/_kill-orphan-conhost.mjs` —— **只杀父进程已消失的** conhost；活父的**一律不动**。

---

### 40. ★★★ 派生物必须被门覆盖，且判据要落在【用户真正碰的那个对象】上

**2026-09-26 真事故（与本轮铁律 39 同一晚，但病因不同）**。用户逐字贴回：
```
'-click' 不是内部或外部命令，也不是可运行的程序
'icon' / 'equired' / 'witchboard.cmd).' / '-' / '-25' …
系统无法接受输入的时间。
输入新时间:
```
...
| | 非 ASCII | 纯 LF | 症状 |
|---|---|---|---|
| 桌面 `DSH 启动 (双击).cmd` | **243** | **66** | ★ 炸（15 行输出）|
| 仓库 `scripts/dsh-up.cmd` | 0 | 0 | ✓ 干净（2 行输出）|

⇒ **源文件修对了，但它是【派生物】的来源；用户双击的是【派生物】那份，而那份从没重新生成。**

**症状 → 病因逐字对应**：`'-click'`←"double-click" / `'icon'`←"icon" / `'equired'`←"required" /
...
旧门 `check-cmd-lineendings.mjs` 只扫仓库内，桌面在 `C:\Users\<user>\Desktop\` ⇒ 不在范围。

**落法**：
1. 门必须覆盖**派生物**（不只源文件），用**指纹重算 + 逐字节比较**（铁律 18），
   三态 `PASS / FAIL / SKIP`（派生物不存在要**显式 `skip`**，既不当"通过"也不当"失败"）。
2. **生成器自己把关**：安装器**拒绝装坏源**（非 ASCII ⇒ 拒绝；纯 LF ⇒ 自动归一，因为拒绝会让桌面
   留着**更坏的旧版**），写完**立刻复验**并把指纹打进日志。

**本仓库现状**：`check:all` **15 → 17 道**，新增 `cmd-lineendings`（判据 A 全 CRLF / B 纯 ASCII）
与 `desktop-launcher`。中文说明的家 = `scripts/README-dsh-up.md`。

**消融自证**（三臂，桌面按字节备份→还原→复验一致）：
A 改成 LF-only ⇒ 红 ✓ ／ B 塞一个非 ASCII ⇒ 红 ✓ ／ C 换成未打戳源文件 ⇒ 红 ✓；还原后 ⇒ 绿 ✓

★★ **同族推广**：凡"生成/拷贝出来的东西"（桌面图标、`lib/` 产物、`lease.json` 快照、
prompt 里的文档副本）都要问一句：**它的门在哪？用户碰的是它还是源？**

---

### 附：`.cmd` 的两条判据为什么成立（环境事实，已进 MEMORY.md 环境约束 23）

- **判据 A（全 CRLF）**：LF-only 会让 `cmd.exe` **吃掉 `rem` 前缀**，把注释里的英文单词当命令执行。
- **判据 B（纯 ASCII）**：`cmd.exe` 按**当前 OEM 代码页**（本机 936）**流式读取并切分**，
  而文件里的 `chcp 65001` **要等被读到才生效** ⇒ 非 ASCII 字节会让字节边界落错位置，
  偶发切出相邻 ASCII 片段当命令执行。
  ★ 实测复现率 **1/25 ≈ 4%**（同文件、同参数、同 cwd）⇒ **双击窗口一闪而过时根本发现不了**，
  且**不改变退出码**（`EXIT=0`）⇒ 任何只看退出码的检查都抓不到它。
- **★ 读 `.cmd` 输出必须 `new TextDecoder('gbk')`**。我按 utf8 解码，中文病征全变乱码 ⇒ 正则匹配不上 ⇒ 假绿。
  判据要**两路并存**：中文句式 + 与编码无关的英文片段。
- **★ 测含空格的路径必须加引号** —— 不加会被空格切开、**根本没执行到目标**，
  报出来的 `'C:\Users\Admin\Desktop\DSH' 不是内部或外部命令` 是"**我调用错了**"的信号，
  **不是被检对象的问题**。我把这两件事混为一谈过。
- **★ 本机 `wmic` 已不存在** ⇒ 读进程改用 `Get-CimInstance Win32_Process`，
  且**必须从 node 脚本文件里调 PS 绝对路径**（bash 里提到 PS 路径会被安全策略拦）。
