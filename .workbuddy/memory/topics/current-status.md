...
## 【更正 + 修复】hard-switch 写入竞态 —— 真因不是 hard-switch（2026-09-16 00:40）

**对我自己上一条结论的更正**：`session-3d8ea18d` 那次损坏（09-15 22:30:37）**不是** hard-switch。
`state.jsonl` 明写 `freeze a lastSeq=0`（旧代**有响应**），且全部历史上**只有 1 次** hard-switch（09-14 08:28:48）。
真因是：**`freeze` 从来不停写**。四个机制全都没生效，且每个都有实测统计：

| 机制 | 实测事实 |
|---|---|
| `drain.evaluateStatic` | 判据 `armed && lastSeq>=0`，**没有任何"停"动作** —— 注释里的意图从未实现 |
| `computeCaughtUpSeq` | **171 条 freeze 记录，全部 `lastSeq=0`** |
| defer 的 `turnInFlight` | **204 次 defer，`waitedTurnEnd=true` 出现 0 次**（22:30 明明有回合在跑却报 `reason=idle`）⇒ `ctx.on('turn/start')` 监听**从未触发** |
| `retainMs=30s` | flip 后旧代继续活着，正在跑的那一轮写到自然结束（实测 flip 后 **+8.6s**） |

**机制（决定性的一条）**：`dsh-session` 的 `seq` 就是 `log.length`；从磁盘载入会话用存储事件当种子
（`repair.js` 合成 closers 时 `let seq = last.seq + 1` 且 `time = last.time`，构造函数再补一条 `session/end-seed`）。
旧代的内存计数器**恒领先于磁盘前缀**（写而未 flush）⇒ **任何第二个加载同一会话的实例，都从旧代即将要写的位置排号**
⇒ **必然重叠**（不是小概率竞态）。现场行号完全对上：见 `docs/handover-vs-restart.md` §8.2/§8.3；
复扫脚本 `out/probe-hsw-race.mjs`（只看"回退点数 / 顶层 seq 重复值"两节）。

**已修（已验证到"门+构建"，未真机验收）**：
- gen 侧 `freeze` 真停（`packages/switchboard/src/drain.ts` 重写）：直读 `agent.phase.kind` → 对在跑的回合
  `cancel({kind:'hook',reason:'handover/freeze'})` → 有界 `whenIdle()` → 官方 `sessions.flush(session)` →
  真 `lastSeq`（live 会话 `seq-1` 的最大值）+ **全量** live 会话 + `primarySessionId`；
  判据 `quiesced` **要求"能看到 agent"** —— "看不见"不是"没在跑"（最容易长出来的假绿）。
- 控制面 `sealPlan`（`coordinator.ts`）：`quiesced=false`（强切 / drain 超时 / 落盘失败 / 看不到 agent）⇒
  **在释放前门锁之前强杀旧代并等 PID 消失**（锁期间前门对 WS/非 GET 一律 503 ⇒ 浏览器无法在旧代还活着时
  把同一份会话加载到新代上）；杀不干净 → 落盘 `unfenced-old-gen.txt` + 大字告警，绝不静默。
- `resumeId` 顺序改为 `primarySessionId`（冻结时**有回合在跑**那个会话）→ 前门嗅探 → seq 最大者。
  原先前门优先，那次把 resume 指到了没在跑的 `session-02a1a38b`。
- `prepareSwitch` 改用真实阶段 ⇒ defer 从"从未生效"变成**真的会**等在跑的回合。
- 门：`scripts/test-handover-drain.mjs`（36 项：红/绿双向 + 封口顺序两方向自证 + 上游形状断言），
  已并入 `check:all` ⇒ **15/15 全绿 13.6s**。
- 顺带修：`packages/switchboard/scripts/build.mjs` 不再依赖全局 `tsc`（改用仓库内
  `node_modules/typescript`）—— 否则本机 shell 里根本构建不了（报"tsc 不是内部或外部命令"）。

**仍未闭合（别当成已修）**：
1. **真机换代没跑过**。验收听 `docs/handover-vs-restart.md` §8.7：`state.jsonl` 里该次交接应有
   `freeze a lastSeq=<非0> quiesced=true (canSeeAgents=true running=N→cancel=N …)`；`gen-<最新>/resume.jsonl`
   里应有 `{"phase":"drain",...}`；`check-session-integrity.mjs --limit 12` 无新增 gap。
2. `readCaughtUpSeq` 的 ready / 首段追平门槛仍是**惰性**的（已改为 1.5s 有界观测项，不再拖 40s）。
3. 回滚路径的重叠窗口仍在（`quiesced=false` + flip 后 verify 失败时）。
4. `session-3d8ea18d` 的 seq gap 仍在（截断会丢 59%）。
