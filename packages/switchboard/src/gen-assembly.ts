...
/**
 * ★★ 2026-09-25 新增：**训练场身份 ⇒ 臂间隔离层**的 overlay 片段。纯函数，可单测。
 *
 * 背景（用户当天的裁决，见 `docs/training-ground-and-skill-sieve-2026-09-25.md` §1）：
 * 「臂」的语义改成「**训练场**」= ① 隔离的数据 ② 记录的消耗与结果 ③ **锚定参与者是谁**。
 * 而 dev 模式（`DSH_PERMISSION_MODE=danger-full-access`）把沙箱**全开**
 * ⇒ 那一层放开之后，**唯一**还拦着"串场"的就是 `packages/arm-isolation` 这个**工具层护栏**。
 *
 * 本函数只做"把身份翻译成装配"：读 `DSH_ARM_SELF`（训练场名）+ `DSH_ARM_DENY`
 * （**逗号分隔的绝对根**：其它训练场的 cwd/store）⇒ 渲染**覆盖** `id: arm-isolation` 的那条 config。
 *
 * ★ 为什么用 `extraDeny` 而不是 `arms`：控制面**不该**去解析臂注册表（那是脚本侧资产）——
 *   让调用方把"要挡哪些根"算好传进来，少一层命名空间耦合（`arms`+`self` 的语义留给插件自己）。
 * ★ 为什么两样**必须同时给**：`arm-isolation` 对"设了 `self` 却无可挡的根"是**抛错**（fail-closed）
 *   ⇒ 只给一半会让**那一代起不来**。所以缺一 ⇒ 返回空串（不插条目），由调用方如实记账。
 *
 * ★★ 2026-09-25 修：**不能再 `insert`** —— 实测首跑就死在这条上。原因（本项目铁律 2 的原样重演）：
 *   隔离实例的 profile 把该包列进了 `dsh.profile.bundles` ⇒ **包自带的 `cordis.patch.yml` 已经
 *   `insert` 过 `id: arm-isolation`** ⇒ 这里再 `insert` 同一个 id ⇒
 *   `duplicate loader entry id: arm-isolation` ⇒ **整树装配失败**。
 *   · ★ **症状指纹**：**控制面 `?cmd=status` 200、前门 502** —— 控制面自己是好的，死的是它后面那一代。
 *   · ★ **正解**：按本项目既有写法**覆盖已有条目的 config**（`- id: <已存在的 id>` + `config:`），
 *     与 `profiles/web/cordis.patch.yml` 里覆盖 `agent-default-model` 是同一个形状。
 *     铁律 2 原话："profile 的 `cordis.patch.yml` 不得写包内已 `insert` 的同一 id ⇒ 整树装配失败"。
 */
export function renderArmIsolationOverlay(self: string | undefined, denyCsv: string | undefined): string {
  const s = (self ?? '').trim()
  const roots = (denyCsv ?? '')
    .split(',')
    .map((x) => x.trim())
  ...
  const out: string[] = []
  out.push('# generated by switchboard · gen-assembly（训练场身份 ⇒ 臂间隔离层）')
  out.push(`# self=${s} denyRoots=${roots.length} 条`)
  out.push('# ★ 用【覆盖 config】而不是 insert：该 id 已由包自带的 cordis.patch.yml insert 过')
  out.push('  # （写成 insert 会 duplicate loader entry id ⇒ 整树装配失败，实测踩过）')
  out.push('- id: arm-isolation')
  out.push('  config:')
  out.push(`    self: ${y(s)}`)
  out.push('    extraDeny:')
  out.push(...lines('      - ', roots.map((r) => y(r))))
  return out.join('\n')
}
...
export function resolveGenSpawnSpec(opts: {
  ...
  // ── 训练场身份 ⇒ 臂间隔离层（2026-09-25 新增）──────────────────────────────────
  // 见 docs/training-ground-and-skill-sieve-2026-09-25.md §1/§2：
  // dev 模式把沙箱全开之后，**这一层是唯一还拦着"串场"的护栏** ⇒ 身份齐备就必须挂上。
  // ★ 身份从 env 来（调用方 = 起隔离实例的那一侧算好），控制面**不解析**臂注册表。
  const armSelf = process.env['DSH_ARM_SELF']
  const armDeny = process.env['DSH_ARM_DENY']
  let armIsolationPatch: string | null = null
  const armYaml = renderArmIsolationOverlay(armSelf, armDeny)
  if (armYaml) {
    mkdirSync(opts.genDir, { recursive: true })
    const f = join(opts.genDir, 'arm-isolation-overlay.yml')
    writeFileSync(f, armYaml, 'utf8')
    armIsolationPatch = f
    const n = (armDeny ?? '').split(',').filter((x) => x.trim()).length
    console.log(
      `[switchboard:arm] ★ 训练场身份 self=${(armSelf ?? '').trim()} ⇒ 挂【臂间隔离层】（要挡 ${n} 个根）`,
    )
  } else if (armSelf || armDeny) {
    // ★★ 半个身份 = 危险，所以**绝不静默**：只给 self 而无可挡的根会让 `arm-isolation` **抛错**
    //    （fail-closed）⇒ **那一代根本起不来**。这里明说"没插条目"，并指出后果。
    console.log(
      `[switchboard:arm] ⚠️ 训练场身份**不完整**（DSH_ARM_SELF=${armSelf ? '有' : '(缺)'} / ` +
        `DSH_ARM_DENY=${armDeny ? '有' : '(缺)'}）⇒ **不插 arm-isolation 条目**：` +
        `这一代**不会被臂间隔离层保护**（沙箱全开时这是唯一护栏）。★ 调用方请把两样给全。`,
    )
  }
  ...
  return {
    ...
    // ★ 顺序 = 清单声明的 → 本模块渲染的池/模型 overlay → **训练场隔离层**（最后 ⇒ 覆盖力最强）
    extraPatches: [
      ...declared,
      ...(overlay ? [overlay] : []),
      ...(armIsolationPatch ? [armIsolationPatch] : []),
    ],
    envExtra,
