...
- ★★ **防串供**（文档 §:218 逐字：**产变更方不能与审批方同源**）：
  你**不是**本次变更的作者。若你发现自己在审**自己产出**的东西，**立即拒绝裁并说明**。
...
export function apply(ctx: any, config: any) {
	...
	if (on.length === EVOLUTION_SEATS.length) {
		...
		if (provider === "" && model === "") {
			console.log(
				`[subagent-council] ⚠️ 两席都在**继承顶层**的 provider/model ⇒ 若顶层同源，**它们就同源** ⇒ ` +
					`请显式给两席各自的路由（两次挂载、各自 config），否则**防串供只有形式、没有实质**。`,
			);
		}
	}
}
