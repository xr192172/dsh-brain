...
### 7.1 三级公民（**判据**，用 `SkillNode` 的现成字段表达）

| 等级 | 定义（用户口径） | 落成字段判据 | 用途 |
|---|---|---|---|
| **一等** | **所有字段齐全**；**或者「有工具 + 有指引」** | `Tools[]` 非空 **∧** `Principle`（指引）非空；<br>或：**必填字段全齐**（★ 合格线见 §7.3 待确认） | **构建子 agent** |
| **二等** | **内置了一段代码片段** + **有描述** | `Script`（+`ScriptLang`/`Archive`）非空 **∧** `Principle`/描述 非空，**但 `Tools[]` 为空** | **也可以构建子 agent** —— ★ 因为"**那段代码片段我们也可以自己手动将其做成工具**" ⇒ **升格路径 = 人工把 `Script` 包成一个 `ToolDef`**，包完就变成一等 |
| **三等** | **只有指导** | `Principle`/描述 非空，**`Script` 空 且 `Tools[]` 空** | ★ **不用来建 agent** ⇒ 用来**优化同类型【已有】子 agent 的提示词** |
...
2. ★★ **「用过的打标记」**：用过的 skill **打上已用标记**，**后续不再碰**。
   ...
   · "不再碰"= 筛/优化流程**跳过** `UseCount ≥ 1` 的节点（★ 待确认是否要一个显式的 `picked` 标记，见 §7.3）。
3. ★ **一等/二等共用一条出路**：都能建 agent；差别只在**二等需要先人工把代码片段做成工具**（这一步是**人做**，不是自动）。

### 7.3 ★ 我剩下的疑问（**要你拍一下**；每条我都给了提议，你点头或改）

1. **"所有字段齐全"的合格线是哪些字段？**
   `SkillNode` 有 ~30 个字段（含 `MergedFrom`/`AbsorbedBy` 这类**谱系/历史**字段），
   "全齐"字面理解几乎不可能达到。⇒ **我的提议**：合格线 = **`ID` + `Principle` + `Triggers[]` + `Tools[]`** 四项非空
   （谱系/历史类**不算**）。**要不要就按这四条？**
2. **公民级 vs 已有的 `Level`（`0=archived / 1=普通 / 3=L3(score≥0.7 ∧ use_count≥10)`）是一根轴还是两根？**
   ⇒ **我的提议**：**两根轴，互不覆盖** —— 公民级 = **结构完整度（静态、看字段）**；
   `Level` = **战绩（动态、看 score/use_count）**。**同意吗？**
3. **三等的"优化提示词"落在哪个字段？副作用判据就用 `SuccessRate` 吗？**
   ⇒ **我的提议**：改的是 **`persona`（agent 侧）**，**不是** skill 的 `Principle`（skill 是配方，不该被反向改）；
   副作用判据用 **`SuccessRate` 掉 + 走已有的 `Demoted` 规则**。**同意吗？**
4. **筛/优化的触发时机？**
   ⇒ **我的提议**：**按需**（建 agent 前 / 优化某个 agent 前各跑一次），**不是定期全量**；
   且**用过的（`UseCount ≥ 1`）直接跳过**（对应你说的"后续不用再碰"）。**同意吗？**

★ 这四条一确认，接缝②就能**直接写成可执行脚本**（判据是纯函数级：给定 `SkillNode` ⇒ 返回 `一等/二等/三等/跳过`）。
