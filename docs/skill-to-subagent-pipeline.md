...
1. ~~**一个未解释的反例**：agnes 的两例 `system` 公共前缀仅 2% 却命中 89.6% / 89.8%~~（原判作废）
   ★ **已裁定（用户，2026-09-21）：不是反例，是【读数不该用】**。
   agnes 是本地网关（`~/.dsh/settings.yaml:5-14`，`http://127.0.0.1:3101/v1`），**免费、只跟次数有关**
   ⇒ **命中率没有经济意义**；我们本来就是为 DeepSeek 打造的 ⇒ **"DeepSeek 算法兼容、命中率高，那才是真的高"**。
   **⇒ 全部非 DeepSeek 样本从结论中剔除**（脚本已按 `model === 'deepseek-v4-flash'` 过滤，见 `out/delegation-reuse.txt` 的 (B)/(C) 两表）：
   **(C) 被剔除的 4 例全是 agnes**（`3333bdd5` / `638c62ce` / `85fcf47d` / `d0414827`）；
   **(B) 只看 DeepSeek（n=10）后 ⇒ 反例消失**：
   · 工具数**相同** ⇒ 高命中 **5 / 低命中 0**；工具数**不同** ⇒ 高命中 **0**（中 2 / 低 2）
   · `system` **逐字完全一致** ⇒ 高命中 **5 / 低命中 0**；不一致 ⇒ 高命中 **0**
   · `provider` = fork（3 高 / 1 低）、spawn（2 高 / 2 中 / 2 低）⇒ **不是干净分割**（组内既有高又有低）
   ⇒ **LCP 模型在 DeepSeek 上完全自洽，没有反例。**（原判"必要但不充分"**撤回**。）
...
**`docs/subagent-model-restatement.md`** —— "现有子代模型"的**逐条复述**（挂了几条委派工具、一次委派的完整生命周期、
脸的四层决定、fork vs spawn、深度与责任链、权限钉、结果通道、成本与缓存，以及**与你构想的逐条对照表 + 我不确定清单**）。
**改动子代机制前先读它**，不要在本文里另起一份现状描述（两处描述必然漂移）。
