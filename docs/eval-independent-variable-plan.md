...
**★★ 为什么"迟到挂载"在这里不是残余风险（要点，别漏）**：

`eval-run.mjs` 的 `stableWindow` 注释里记着一条**历史观测**：
早期 `exp-base-nodc` 被探到 **30**，跑中却**变成 76**，因为
**MCP server 是独立进程、会迟到把工具挂上来** ⇒ 当时的臂隔离**实际失败了**。

⇒ 那条观测发生在**只断了①(包)、②(insert) 还在**的配置上（boot.log 有 `MCP server started`）。
**今天这一臂的差别不是"没观察到迟到"，而是"迟到在因果上不可能"**：
通道② 显示该代 boot.log **没有 `mcp-client` 装载项**、**`MCP server started` 出现 0 次**
⇒ **那个 server 从未被拉起** ⇒ **没有任何东西能在稍后挂工具上来**。

⇒ 这就是为什么**必须两条通道**：单看"工具面 0"永远只是"没观察到"（窗口再长也只是"更久没观察到"）；
**加上"server 从未启动"才把结论从"未观测"升级成"不可能"**。
（且今天的窗口 10–15s，**短于**历史上迟到被发现的时延 ⇒ 更要靠通道②，不能靠加长窗口。）
