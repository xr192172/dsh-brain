...
## ★ 重要发现：**那三题已饱和（成功率无区分度）—— 但 `cli-0004` 例外**

> ★ **2026-09-21 补充**：本节原标题是"**这批题已经饱和，不能用成功率当主判据**"，
> 那是当时（3 题 + `cli-0005`）的结论。**后来落的 `cli-0004` 不饱和** ——
> 它 **oracle 绿但超预算 7 秒** ⇒ **成功率维度从它开始才有信息量**（见文末「补跑：`cli-0004`」）。
> ⇒ 所以现在的口径是：**前 3 题 + `cli-0005` 当"地板"（两臂都必须 FIXED），
> 而"成功率是否分化"要看 `cli-0004`（以及后续新增的能力题）。**
> （改这条是因为**本文档里两条断言互相冲突** ⇒ 至少一条错 ⇒ 去核实后改掉过时的那条。）
